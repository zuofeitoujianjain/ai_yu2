...
- **「停」字停止对话**：群内消息出现 `stop_keywords`（默认"停"）→ 立刻**清空全群免@窗口并静音**（只回一句确认）；之后免@/关键词都不再自动回，**被 @ 即解除静音**恢复对话
- **指令系统**：`/help`、`/人设 <名>`、`/画像`、`/记忆`、`/开关 <功能>`
- **人设切换接口**：`@机器人 切换人设 人设名` 一句话切换人格

### 🧠 记忆与人格（B / D 组）
- **独立人设文件**：所有人设放在 `personas.json`（`{ "人设名": "系统提示词" }`），**加人设不用改代码**，编辑保存约 15 秒热生效。**初始人设**是其中的 `默认` 条目（`config.json` 的 `system_prompt` 仅作后备）
- **人设隔离**：会话上下文 key = `群:人:人设`、长期记忆 key = `(群,人,人设)` → **不同角色的对话与记忆互不关联**，切到塔菲不会"记得"你跟鲸鱼娘聊过什么
- **多角色 / 多群独立人设**：每群可独立切换人设（内置 `默认`(邪恶鲸鱼娘) / `高冷` / `永雏塔菲`）
- **用户画像**：记录每人各群的**群名片 + 常用话题**（bigram 词频），对话时注入「资料卡」让模型"认识"对方
- **独立会话记忆**：按「群 + 人 + 人设」维护最近 N 轮上下文；「清空记忆」关键词重置
- **长期记忆**：`memory.json` 持久化 + **bigram 相似度召回**，**跨会话/重启**记住前因后果；`/记忆` 查看、`/记忆 清空` 清空你名下所有人设的记忆

### ⚡ AI 能力进阶（C 组）
- **智能路由**：简单问题走快模型（如 `deepseek-flash`），命中关键词或超长自动切思考模型（如 `deepseek-v4-pro`）；可用 `router_enabled: false` 关闭路由、全程走单一模型（模型名取决于你的 API 权限，`GET /models` 可查）
...
├── personas.json          # 独立人设库：{ "人设名": "系统提示词" }，「默认」=初始人设（可直接编辑添加）
├── memory.json            # 长期记忆持久化（运行时生成，按 群|人|人设 隔离；已在 .gitignore）
...
| `deepseek_model` / `model_reasoner` | `deepseek-flash` / `deepseek-v4-pro` | 快速 / 思考模型（路由用；可用模型取决于你的 API 权限，`GET /models` 可查） |
| `system_prompt` | 鲸鱼娘人设 | **后备**默认人设（当 `personas.json` 里没有「默认」时才用） |
| `personas_file` | `personas.json` | **独立人设库文件**（推荐在此编辑/添加人设，热生效） |
| `personas` | `{}` | 配置内联人设（一般留空；文件优先） |
| `trigger_keywords` | `[鱼, 鲸鱼]` | 关键词触发列表 |
| `stop_keywords` / `stop_reply` | `["停"]` / … | **「停」字停止对话**的触发字与确认语 |
...
> **热更新**：改上述**行为类**配置后约 15 秒生效（无需重启）；仅连接/密钥类（`ws_url`/`token`/`bot_qq`/`api_key`）需重启。`personas.json` 被**单独监视**，人设改完即时生效。
...
| `@我 切换人设 <名>` | 一句话切换本群人设 |
| `/画像` | 查看你的资料卡 |
| `/记忆` · `/记忆 清空` | 查看 / 清空你名下所有人设的长期记忆 |
| `/开关 <关键词·指令·私聊>` | 开关功能 |

### 人设文件 `personas.json`（加人设不用改代码）

所有人设集中在这一个文件里，直接编辑即可（保存后约 **15 秒热生效**）：

```json
{
  "默认": "你是「邪恶鲸鱼娘」……（初始人设，未切换时使用）",
  "高冷": "你是高冷的机器人，说话简短冷淡，不爱废话。",
  "我的人设名": "这个角色的系统提示词，随便写"
}
```

- **初始人设** = `默认` 条目（想改初始人格就改它；`config.json` 的 `system_prompt` 仅作后备）
- **切换**：`@机器人 切换人设 <人设名>` 或 `/人设 <人设名>`；切回初始用 `默认`
- **人设隔离**：不同人设拥有**独立的会话上下文与长期记忆**，互不串味

### 「停」字停止对话

群里出现 `stop_keywords`（默认"停"）→ 机器人**立刻停止本群所有对话**：清空全群免@窗口 + 本群静音（免@/关键词都不再自动回），只回一句确认；**被 @ 即解除静音**恢复。触发字与确认语可在配置里改。
